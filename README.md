...
- **登录与权限**：JWT；首次启动自动创建管理员账号；管理员可创建子账号；错题数据按用户隔离。
...
**默认管理员账号**：首次启动会自动创建用户名 `admin` 的管理员。登录页不展示演示密码；使用 Docker 镜像部署时，容器每次启动会在**标准错误输出**中打印默认密码提示，便于安装后登录。查看方式示例：
...
日志中会出现类似：`【AI 错题本】默认管理员：用户名 admin，密码 123456。` 生产环境请尽快修改该密码。本地 `uvicorn` 开发启动不会在控制台打印该提示。
